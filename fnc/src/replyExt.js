...
module.exports = function(message, text, { mention = true, del = true, delay = 5, color } = {}) {
	...
	return message.channel.send({
		...
		if (del) {
			try {
				...
				if (e.name === 'DiscordAPIError' && e.message === 'Missing Permissions') return message.channel.send(`${message.guild.owner}, I don't have permission to add reactions here!`);
			}
		}
	});
};
