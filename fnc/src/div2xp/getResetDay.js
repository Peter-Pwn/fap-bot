...
module.exports = function(date = moment()) {
	...
	date.utc();
	if (date.isoWeekday() < CON.DIV2XPRST.DAY || date.isoWeekday() === CON.DIV2XPRST.DAY && fnc.getTime(date).isBefore(fnc.getTime([CON.DIV2XPRST.HOUR, CON.DIV2XPRST.MINUTE], true))) {
		date.subtract(1, 'w');
	}
