...
module.exports = {
	...
	async execute(message, args) {
		...
		try {
			...
			if (args.length) {
				channel = client.channels.cache.find(c => c.name === args[0] && c.type === 'voice');
			...
			let lock = client.locks.get(channel.id);
			if (lock) {
				...
				await db.locks.create(lock);
				client.locks.set(channel.id, lock);
